...
pub(crate) fn complete_restore_preflight_job(
    connection: &Connection,
    db_path: &Path,
    job_id: &str,
...
pub(crate) fn mark_task_waiting_for_input(
    connection: &Connection,
    job_id: &str,
    task_key: &str,
    message: &str,
...
pub(crate) fn mark_task_blocked(
    connection: &Connection,
    job_id: &str,
    task_key: &str,
    message: &str,
...
pub(crate) fn load_task_summaries(
    connection: &Connection,
    job_id: &str,
...
pub(crate) fn load_latest_artifact(
    connection: &Connection,
    job_id: &str,
...
pub(crate) fn load_backup_artifact(
    connection: &Connection,
    backup_id: &str,
) -> Result<JobArtifactSummary> {
    let mut statement = connection.prepare(
        "SELECT id, artifact_kind, uri, label, metadata_json, created_at
...
pub(crate) fn validate_manifest_shape(manifest: &BackupManifest) -> Result<()> {
...
pub(crate) fn canonical_manifest_path(path: &str, boundary: &Path, label: &str) -> Result<PathBuf> {
...
pub(crate) fn ensure_path_within(path: &Path, boundary: &Path, label: &str) -> Result<()> {
...
pub(crate) fn sqlite_integrity_check(connection: &Connection) -> Result<String> {
    let result: String = connection.query_row("PRAGMA integrity_check", [], |row| row.get(0))?;
    Ok(result)
}

pub(crate) fn snapshot_database(connection: &Connection, target_path: &Path) -> Result<()> {
...
pub(crate) fn archive_scanned_files(
    data_dir: &Path,
    archive_path: &Path,
...
pub(crate) fn verify_archived_files(manifest: &BackupManifest, archive_path: &Path) -> Result<()> {
...
pub(crate) fn ensure_safe_relative_archive_path(path: &Path) -> Result<()> {
...
pub(crate) fn acquire_backup_lock(backups_dir: &Path) -> Result<BackupLock> {
...
pub(crate) fn checksum_manifest_payload(manifest: &BackupManifest) -> Result<String> {
...
pub(crate) fn format_checksum(bytes: &[u8]) -> String {
    format!(
...
pub(crate) fn data_dir_for(db_path: &Path) -> PathBuf {
    db_path
        .parent()
...
pub(crate) fn backups_dir_for(db_path: &Path) -> Result<PathBuf> {
    let backups_dir = data_dir_for(db_path).join("backups");
    fs::create_dir_all(&backups_dir)?;
    Ok(backups_dir)
}

pub(crate) fn elapsed_seconds(started_at: Option<&str>, completed_at: Option<&str>) -> Option<i64> {
...
#[cfg(test)]
mod tests {
    use super::super::api::{create_backup, run_restore_preflight};
    use super::*;
