...
mod tests {
    ...
    fn conversation_analysis_tables_are_created() {
        ...
        assert!(column_exists(&connection, "schedules", "cron_expression"));
        assert!(column_exists(
            &connection,
            "artifact_patch_proposals",
            "patch_text"
    ...
    #[test]
    fn content_analytics_event_tables_are_created() {
        let connection = Connection::open_in_memory().unwrap();
        init_schema(&connection).unwrap();

        assert!(table_exists(&connection, "content_analytics_events"));
        assert!(column_exists(
            &connection,
            "content_analytics_events",
            "event_kind"
        ...
        assert!(index_exists(&connection, "idx_content_analytics_content"));
        assert!(index_exists(&connection, "idx_content_analytics_source"));
    }
