...
0.1.8 Interactive Account And LLM Chat should not invent a second conversation
protocol for member chat or assistant responses. Local session, chat bootstrap,
message submission, deterministic LLM requests, guarded live-provider requests,
and replay-safe UI should use HTTP route/read-model contracts for bootstrap
state and `/chat/ws` for relationship conversation commands and replay where
realtime interaction is needed. Any new event semantics should persist through
the existing SQLite domain tables and conversation/realtime event logs.
